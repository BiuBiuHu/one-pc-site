...
// --- 3. 生成封面 prompt（高端设计工作室审美）---
const prompt = `为中文技术博客设计一张封面，审美标准是「高端设计工作室出品」，不是普通 AI 插画。
...
1. 风格：极简主义 / 瑞士设计（Swiss Design），理性、克制、有秩序感。禁止 soft 3D、clay render、Dribbble 式 SaaS 插画、背身无脸小人。

2. 概念：只用 1 个精准的抽象视觉隐喻表达主题，不要堆砌多个符号。宁少勿多，留有余味。

3. 配色：严格 3 色以内——一个深色锚点（深藏青 #1B2B4A 或深墨），一个强调色（只用一点点作点睛），其余大量浅灰白留白（#F4F5F7）。禁止「青绿→蓝紫」这类 AI 出厂默认渐变。

4. 几何：图形边缘锐利、精确、对齐。正圆是正圆，线宽一致，刻度均分，元素间距有网格感。

5. 质感：加 2-4% 细微噪点颗粒，呈现「印刷质感」而非磨砂模糊。禁止整体模糊、过度平滑的塑料渐变。

6. 光影：要么彻底扁平（无投影，靠色彩分层造深度），要么单一光源 + 一致投影。禁止各自为政的乱投影。

7. 留白：大量留白，且有「设计过的」网格感和节奏，不是空着。

8. 构图：16:9，主体居中或偏左，右侧留白呼吸。

9. 禁止：任何文字、字母、数字、水印（避免乱码）；粗渐变箭头；SmartArt 式元素；多个场景堆砌。`;
